...
import com.example.demo.qwen.tools.CourseTools;
...
import com.example.demo.qwen.costants.SystemConstants;

@Configuration
public class AiConfig {

    /**
     * 注册 ChatMemory Bean，用于多轮对话上下文存储
     */
    @Bean
    ...
    /**
     * 注册 ChatClient Bean
     * 使用 ChatClient.Builder 来构建 ChatClient，并配置默认行为
     */
    @Bean
    public ChatClient chatClient(ChatClient.Builder builder, ChatMemory chatMemory) {
        return builder
                .defaultSystem("你是Java程序员的开发助手，名字叫lsj")
                .defaultAdvisors(
                        new SimpleLoggerAdvisor(),
                        new MessageChatMemoryAdvisor(chatMemory) // 启用记忆功能
                )
                .build();
    }
    ...
    @Bean
    public ChatClient serviceChatClient(ChatClient.Builder builder, ChatMemory chatMemory, CourseTools courseTools) {
        return builder
